...
def get_calculation(raw, meta, norm=False, norm_ct=None, threshold_method="stdv", time_skip=5,
    ...
    #Helper: Max-point ratio = peak/background
    def calculate_mpr(raw, background):
        return raw.max(axis=0) / background

    #Helper: Compute threshold using chosen method
    ...
    #Helper: Rate of amyloid formation
    def calculate_raf(raw, threshold, time_skip):
        time_to_threshold = []
        for column in raw.columns:
            #Find first index after 'time_skip' where signal exceeds threshold
    ...
    #Step 1: Get background signal (used for thresholding)
    ...
    #Step 4: Create output DataFrame with all metrics
    ...
    #Step 5: Normalize metrics using control group
    if norm:
    ...
    #Step 6: Flag if a sample crossed the threshold
